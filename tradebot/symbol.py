...
class Symbol:
    def __init__(self, name, config, ref=None):
        ...
        self.take_profit = config.get('take_profit', ref.take_profit if ref else 0.0)
        self.stop_loss = config.get('stop_loss', ref.stop_loss if ref else 0.0)
        ...
        self.size = config.get('size', ref.size if ref else None)
